...
class main_test(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.Res_power_cal0 = os.path.join(self.config_path, 'Res_Optical_calibration_ITLA_CH0_5wavelengthC++_20221111_153500.csv')
    ...
    def Start_test(self):
        ...
        if self.test_type=='Normal':
            if self.test_flag=='DC':
                if self.ITLA_type=='C80':
                    #self.channel=[i for i in range(1,81) if np.mod(i,2)==1]
                    wl_toTest=np.int0(np.linspace(1,80,30)).tolist()
                    wl_toTest[5]=13
                    wl_toTest[-4]=72
                    self.channel=wl_toTest
                    self.update_config()
                else:
                    self.channel=[i for i in range(9,73) if np.mod(i,2)==1]
                    self.channel[-1]=72
                    self.update_config()
...
class WorkThread(QThread):
    ...
    def ICR_test_GBS(self):
        ...
        *****Start the whole test steps*****
        ...
        wb = xw.Book(mawin.report_model.replace('test_report.xlsx', 'test_report_ICR.xlsx'))
        #wb = app.books.open(mawin.report_model.replace('test_report.xlsx', 'test_report_ICR.xlsx'))
        worksht = wb.sheets(1)
        worksht.activate()
        worksht.range((1, 2)).value = test_result.iloc[0, 0]
        worksht.range((2, 2)).value = test_result.iloc[0, 1]
    ...
    def Res_test_GBS(self):
        ...
        *****Start the whole test steps*****
        ...
        for i in range(len(mawin.channel)):
            ...
            # Close the figs
            self.sig_plotClose.emit()
        #set MPC to zero after finish the test
        setCur=0
        mpc.set_vol(mawin,1,setCur)
        time.sleep(0.3)
        mpc.set_vol(mawin,2,setCur)
        time.sleep(0.3)
        mpc.set_vol(mawin,3,setCur)
        time.sleep(0.3)
        mpc.set_vol(mawin,4,setCur)
        time.sleep(0.3)
        # close the laser output
        mawin.CtrlB.write(b'itla_wr 0 0x32 0x00\n');
        time.sleep(0.1)
